...
export function ContactSection({ address, alternativeAddress, coordinates, mapsUrl, phone, workingHours, socialLinks, ctaLabel, context, onRequest }: { address: string; alternativeAddress?: string; coordinates?: { lat: number; lng: number }; mapsUrl?: string; phone?: string; workingHours?: string; socialLinks: SocialLink[]; ctaLabel: string; context: string; onRequest: (context: string) => void }) {
  const validSocials = socialLinks.filter((link) => link.label.trim() && /^https?:\/\//i.test(link.href));
  const rawPhone = phone?.replace(/[^+\d]/g, "");
  return <Section tone="forest" headingId="contacts-heading"><PageContainer><div className={styles.contacts}><div className={styles.contactCopy}><Text as="h2" id="contacts-heading" variant="h2" tone="inverse">Контакты</Text><address><p>{address}</p>{alternativeAddress?.trim() ? <p>{alternativeAddress}</p> : null}</address>{workingHours?.trim() ? <p>{workingHours}</p> : null}{rawPhone || validSocials.length ? <ul className={styles.contactChannels}>{rawPhone ? <li><a href={`tel:${rawPhone}`} target="_blank" rel="noopener noreferrer"><Icon name="phone" size={24} /><span>{/^\+7\d{10}$/.test(rawPhone) ? rawPhone.replace(/^(\+7)(\d{3})(\d{3})(\d{2})(\d{2})$/, "$1 $2 $3-$4-$5") : phone}</span></a></li> : null}{validSocials.map((link) => { const isVk = link.type === "vk" || /^(VK|ВКонтакте)$/i.test(link.label); return <li key={link.href}><a href={link.href} target="_blank" rel="noopener noreferrer"><Icon name={isVk ? "vk" : "instagram"} size={24} /><span>{isVk ? "VK" : link.label}</span></a></li>; })}</ul> : null}<Button onClick={() => onRequest(context)}>{ctaLabel}</Button></div>{coordinates || mapsUrl ? <MapEmbed address={address} coordinates={coordinates} mapsUrl={mapsUrl} /> : null}</div></PageContainer></Section>;
}
