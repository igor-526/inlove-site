...
import { EmptyState, ErrorBlock, Skeleton } from "../feedback";
...
type IntroProps = { eyebrow?: string; title: string; body?: string; image?: ImageSource; imageSide?: "left" | "right"; actions?: Action[]; headingLevel?: 1 | 2; spacing?: "editorial" | "compact" };
export function IntroSection({ eyebrow, title, body, image, imageSide = "right", actions, headingLevel = 2, spacing = "editorial" }: IntroProps) {
  const headingId = `intro-${title.replace(/\s+/g, "-").toLowerCase()}`;
  const Heading = headingLevel === 1 ? "h1" : "h2";
  return <Section headingId={headingId} spacing={spacing}><PageContainer><div className={`${styles.intro} ${imageSide === "left" ? styles.introImageLeft : ""}`}><div className={styles.copy}><SectionLabel text={eyebrow} /><Text as={Heading} id={headingId} variant={headingLevel === 1 ? "h1" : "h2"}>{title}</Text>{body?.trim() ? <Text variant="body-l">{body}</Text> : null}<Actions actions={actions} /></div>{image?.src ? <div className={styles.introMedia}><ResponsiveImage {...image} ratio="16:10" /></div> : null}</div></PageContainer></Section>;
}
...
export function EditorialSplitSection({ eyebrow, title, body, image, imageSide = "left", actions, headingLevel = 2 }: IntroProps) {
  ...
  return <Section headingId={headingId}><PageContainer><div className={`${styles.split} ${!hasImage ? styles.splitFullWidth : imageSide === "right" ? styles.imageRight : ""}`}><div className={styles.copy}><SectionLabel text={eyebrow} /><Text as={Heading} id={headingId} variant={headingLevel === 1 ? "h1" : "h2"}>{title}</Text>{body?.trim() ? <Text>{body}</Text> : null}<Actions actions={actions} /></div>{hasImage ? <div className={styles.splitMedia}><ResponsiveImage {...image} ratio="4:5" /></div> : null}</div></PageContainer></Section>;
}
...
export function NewsSection({ items, total, mode, state = "success", emptyText = "Новостей пока нет", errorMessage = "Не удалось загрузить новости.", archiveHref = "/novosti", pagination, onRetry, onOpen }: { items: NewsSummary[]; total: number; mode: "latest" | "archive"; state?: AsyncState; emptyText?: string; errorMessage?: string; archiveHref?: string; pagination?: { page: number; pending?: boolean; error?: string; label?: string; onLoadMore: (page: number) => void }; onRetry?: () => void; onOpen?: (news: NewsSummary) => void }) {
  return <Section headingId="news-heading"><PageContainer><div className={styles.heading}><Text as="h2" id="news-heading" variant="h2">Новости</Text><TextLink href={archiveHref}>Все новости</TextLink></div>{state === "loading" ? <Skeleton count={mode === "latest" ? 1 : 3} label="Загрузка новостей" /> : state === "error" ? <ErrorBlock message={errorMessage} onRetry={onRetry} /> : state === "empty" || !items.length ? mode === "latest" ? null : <EmptyState title={emptyText} /> : <><div className={styles.newsGrid}>{items.map((news, index) => <NewsCard key={news.id} news={news} variant={mode === "latest" ? "compact" : "vertical"} featured={mode === "latest" && index === 0} href={news.href || (news.slug ? `/novosti/${encodeURIComponent(news.slug)}` : undefined)} onOpen={onOpen ? () => onOpen(news) : undefined} />)}</div>{mode === "archive" && pagination ? <PaginationLoadMore page={pagination.page} loaded={items.length} total={total} pending={pagination.pending} error={pagination.error} label={pagination.label} onLoadMore={pagination.onLoadMore} /> : null}</>}</PageContainer></Section>;
}

export type SocialLink = { type?: "vk" | "instagram"; label: string; href: string };
export type HoursRange = { start: string; stop: string };
function formatHoursRange(range?: HoursRange): string | undefined {
  const start = range?.start?.trim();
  const stop = range?.stop?.trim();
  return start && stop ? `${start}–${stop}` : undefined;
}
export function ContactSection({ address, nearestStop, latitude, longitude, mapUrl, phone, weekdayHours, weekendHours, socialLinks, ctaLabel, context, trimBottom = false, onRequest }: { address?: string; nearestStop?: string; latitude?: number; longitude?: number; mapUrl?: string; phone?: string; weekdayHours?: HoursRange; weekendHours?: HoursRange; socialLinks: SocialLink[]; ctaLabel: string; context: string; trimBottom?: boolean; onRequest: (context: string) => void }) {
  const validSocials = socialLinks.filter((link) => link.label.trim() && /^https?:\/\//i.test(link.href));
  const rawPhone = phone?.replace(/[^+\d]/g, "");
  const weekdayLabel = formatHoursRange(weekdayHours);
  const weekendLabel = formatHoursRange(weekendHours);
  const hasMap = (latitude !== undefined && longitude !== undefined) || Boolean(mapUrl);
  return <Section tone="forest" headingId="contacts-heading" trimBottom={trimBottom}><PageContainer><div className={styles.contacts}><div className={styles.contactCopy}><Text as="h2" id="contacts-heading" variant="h2" tone="inverse">Контакты</Text>{address?.trim() ? <address><p>{address}</p></address> : null}{nearestStop?.trim() ? <p>{nearestStop}</p> : null}{weekdayLabel ? <p>Будни: {weekdayLabel}</p> : null}{weekendLabel ? <p>Выходные: {weekendLabel}</p> : null}{rawPhone || validSocials.length ? <ul className={styles.contactChannels}>{rawPhone ? <li><a href={`tel:${rawPhone}`} target="_blank" rel="noopener noreferrer"><Icon name="phone" size={24} /><span>{/^\+7\d{10}$/.test(rawPhone) ? rawPhone.replace(/^(\+7)(\d{3})(\d{3})(\d{2})(\d{2})$/, "$1 $2 $3-$4-$5") : phone}</span></a></li> : null}{validSocials.map((link) => { const isVk = link.type === "vk" || /^(VK|ВКонтакте)$/i.test(link.label); return <li key={link.href}><a href={link.href} target="_blank" rel="noopener noreferrer"><Icon name={isVk ? "vk" : "instagram"} size={24} /><span>{isVk ? "VK" : link.label}</span></a></li>; })}</ul> : null}<Button onClick={() => onRequest(context)}>{ctaLabel}</Button></div>{hasMap ? <MapEmbed address={address ?? "Клуб «Инлав»"} latitude={latitude} longitude={longitude} mapUrl={mapUrl} /> : null}</div></PageContainer></Section>;
}
