...
type HoursRange = { start: string; stop: string };
...
export function SiteFooter({ shortName, description, menu, address, phone, weekdayHours, weekendHours, mapUrl, socialLinks, copyrightName }: { shortName: string; description?: string; menu: SiteMenuItem[]; address?: string; phone?: string; weekdayHours?: HoursRange; weekendHours?: HoursRange; mapUrl?: string; socialLinks: SiteSocialLink[]; copyrightName: string }) {
  const pathname = usePathname();
  const weekdayLabel = formatHoursRange(weekdayHours);
  const weekendLabel = formatHoursRange(weekendHours);
  const hasHours = Boolean(weekdayLabel || weekendLabel);
  return <footer className={styles.footer}><PageContainer><div className={styles.footerGrid}><div><Logo variant="light" shortName={shortName} />{description ? <p>{description}</p> : null}</div><nav aria-label="Навигация в подвале"><h2>Разделы</h2><GroupedNavigation menu={menu} pathname={pathname} variant="footer" /></nav>{address || phone || hasHours || socialLinks.length ? <div className={styles.contacts}><h2>Контакты</h2>{address ? mapUrl ? <a href={mapUrl} target="_blank" rel="noopener noreferrer">{address}</a> : <p>{address}</p> : null}{weekdayLabel ? <p>Будни: {weekdayLabel}</p> : null}{weekendLabel ? <p>Выходные: {weekendLabel}</p> : null}{phone ? <a href={phoneHref(phone)} target="_blank" rel="noopener noreferrer"><Icon name="phone" size={20} />{phone}</a> : null}{socialLinks.map((social) => <a key={social.type} href={social.href} target="_blank" rel="noopener noreferrer"><Icon name={social.type === "vk" ? "vk" : "instagram"} size={20} />{social.type === "vk" ? "VK" : social.label}</a>)}</div> : null}</div><div className={styles.legal}>© {new Date().getFullYear()} {copyrightName}</div></PageContainer></footer>;
}
