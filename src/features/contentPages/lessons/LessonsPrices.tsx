...
import { LESSONS_GROUPS, type LessonCategory, type loadLessonsData } from "../services/lessonsLoaders";
...
export function LessonsPrices({ prices }: { prices: PricesState }) {
  ...
  const items: TariffSummary[] = useMemo(() => prices.status === "success" ? prices.data
    // Backend-driven categorization (D5): a tariff's own `groups` field, not a hardcoded
    // slug→category dictionary, decides which toggle state it appears under.
    .filter((item) => item.groups.some((group) => group.name === LESSONS_GROUPS[category]))
    .map((item) => ({
      name: item.name,
      slug: item.slug,
