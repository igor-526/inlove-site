...
describe("Lessons SSR content", () => {
  it("renders one h1, default single-category tariffs, benefits and final CTA", () => {
    const data: Data = {
      settings: { status: "success", data: [setting("home.program_benefits", '[{"title":"Опытные тренеры"}]', "object")] },
    ...
    expect(getByRole("heading", { name: "Опытные тренеры" })).toBeTruthy();
  ...
  it("switches to subscriptions via the Разовые/Абонементы toggle", () => {
    const data: Data = { ...empty, prices: { status: "success", data: [single, subscription] } };
    const { container, getByRole } = render(<LessonsContent data={data} />);
  ...
  it("uses seeded SEO then short_name fallback and canonical path", () => {
    ...
    expect(lessonsMetadata({ status: "success", data: [setting("site.short_name", "Инлав-Клуб")] }).title).toBe("Инлав-Клуб | Занятия и абонементы");
    expect(lessonsMetadata({ status: "success", data: [setting("seo.lessons.title", "Занятия — заголовок")] }).title).toBe("Занятия — заголовок");
  });
});
